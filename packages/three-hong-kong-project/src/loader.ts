...
const getModelFileSize = async (modelUrl: string) => {
  ...
  try {
    console.log(
      `[react-three-hong-kong-project]: load model content length. Model url is ${modelUrl}`
    )
    ...
    console.log(
      '[react-three-hong-kong-project]: error to get gltf model file size. Error:',
      error
    )
  }
...
const loadGltfModel = async (
  ...
  try {
    console.log(
      `[react-three-hong-kong-project]: load model async. Model url is ${modelUrl}`
    )
    ...
    console.log(
      '[react-three-hong-kong-project]: error to load gltf model. Error:',
      error
    )
  }
  return defaultModel
